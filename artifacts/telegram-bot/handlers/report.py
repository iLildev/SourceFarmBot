...
def _extract_media(message: Message) -> tuple[str, str | None]:
    """Return (description_text, photo_file_id | None) from any message type."""
    if message.photo:
        desc = message.caption or "(صورة بدون وصف)"
        return desc, message.photo[-1].file_id
    if message.video:
...
def _replied_summary(replied: Message) -> str:
    """One-line summary of the replied-to message for the report description."""
    if replied.text:
        preview = replied.text[:200]
...
async def cmd_report(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    # ── Case A: reply to a bot message ──
    if message.reply_to_message:
        summary = _replied_summary(message.reply_to_message)
        # If the user also added text after /report, include it
    ...
    # ── Case B: /report with inline text ──
    args = (message.text or "").strip().split(maxsplit=1)
    if len(args) > 1:
        description = args[1]
        await _finish_report(bot, message.chat.id, user.id, user_name, description)
        return

    # ── Case C: /report with media (photo/video/doc) ──
    if message.photo or message.video or message.document:
        description, screenshot_id = _extract_media(message)
        await _finish_report(
            bot, message.chat.id, user.id, user_name,
            description, screenshot_id,
        )
        return

    # ── Case D: /report alone → start conversation ──
...
async def receive_description(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    # If the description message already contains a photo → submit immediately
    if screenshot_id:
        await state.clear()
        await _finish_report(
...
async def cancel_report(callback: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    await callback.message.edit_text("❌ تم إلغاء البلاغ.")
    await callback.answer()
